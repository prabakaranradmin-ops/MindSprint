...
**Doc version:** 4.8 · **Status date:** 2026-07-22 (see Changelog, §25)
...
**Naming note:** "Bloom Academy" is a **working title** — a live US trademark conflict was found (Appendix B, D4). **"Puddlejump" was selected 2026-07-22 as the replacement candidate** (O4, `NAME_SHORTLIST.md`) but is not yet formally cleared (USPTO/app-store/domain/attorney steps still pending) — this document, the codebase, and the app itself all still say "Bloom Academy" throughout and have not been renamed; that rename is separate future work gated on clearance actually completing, not done as part of choosing the candidate name.
