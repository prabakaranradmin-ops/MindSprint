...
- `progress`: per-subject → **per-world** → node states (done/current/locked) + per-stage stars — keyed `(subject, worldId)` even while only world 1 exists. [Shipped]
