...
**Doc version:** 4.17 · **Status date:** 2026-07-22 (see Changelog, §25)
...
| Date | Version | Change |
|---|---|---|
| 2026-07-22 | 4.17 | **Middle science stage swap (§26.14): the 2 skills §26.13 authored but left unused are now (partly) wired in.** `stageConfigsMiddle.science`'s `matter_states` stage (previously duplicated with Senior's own `matter_states` stage) was replaced with `science.life_cycles_traits` (NGSS 3-LS3, traits/heredity), authored in the O11 pass but never assigned to a stage. Considered and rejected a true 6th stage — `NODE_COUNT=5` is a shared constant across every subject/tier's progress-node array and map UI, so bumping it would force Junior's map to 6 nodes too. 2 more difficulty-1 items authored for `life_cycles_traits` (pool was thin at 2, now 4). `science.earth_systems` remains authored-but-unused for a future review. 1 new acceptance test. All 87 acceptance tests and 83 unit tests pass. |
...
### 26.14 Middle science stage swap — wiring the two skills §26.13 left unused (2026-07-22)

§26.13 authored `science.life_cycles_traits` and `science.earth_systems` into the `scienceQuiz` bank "for future stage assignment" but wired neither to a stage config, leaving 2 of the bank's 7 skills genuinely unused in the shipped game.

Adding either as a true 6th stage was considered and rejected: `NODE_COUNT = 5` is a single shared constant driving every subject/tier's progress-node array, the map/stage-picker grid, and "Stage N of 5" UI text — bumping it would force Junior's 5-node map (and every other subject) to 6 nodes too, a materially bigger change than "wire 2 unused skills" implies. Forking a tier-and-subject-specific stage count was also considered and rejected as disproportionate for 2 content items.

**What shipped instead:** `stageConfigsMiddle.science`'s `matter_states` stage (the one skill Middle and Senior duplicated, differing only in difficulty band) was swapped for `science.life_cycles_traits` (NGSS 3-LS3, traits/heredity — a distinct concept from `lifeorder`'s metamorphosis-*order* mechanic already in the same lineup). Senior's `matter_states` stage is untouched, so the concept is still taught once overall, just no longer duplicated across tiers. `science.earth_systems` remains authored-but-unused; the same swap pattern applies whenever a future stage review wants it (e.g. against Senior's own `matter_states` stage, or a Words/Music slot review).

**Content depth check (same discipline as §26.13):** `life_cycles_traits`' pool had only 2 items, below the ~4–5 every other scienceQuiz stage carries — 2 more difficulty-1 items were authored before wiring it in, bringing the pool to 4.

1 new acceptance test confirms the swapped stage renders real content and that "States of Matter" no longer appears at Middle (no longer duplicated with Senior). All 87 acceptance tests and 83 unit tests pass.
