...
**Product:** Bloom Academy — educational game for children ages 5–12: Junior (5–7, original v1 scope), Middle (8–9), and Senior (10–12) tiers — see §26
**Doc version:** 4.3 · **Status date:** 2026-07-22 (see Changelog, §25)
...
Bloom Academy is an original education + infotainment game, originally for children ages 5–7 (Khan Academy Kids-style) and expanded 2026-07-22 (§26, decision O8) to serve ages 5–12 via three age tiers. The **Junior** tier (5–7, the original design) is a mascot-guided experience: **Pip** (a round sprout creature) guides the child through themed subject worlds with node-on-path progression, five distinct activity mechanics, star/coin rewards, an accessory shop, and a parent dashboard behind a math gate. The **Middle** (8–9) and **Senior** (10–12) tiers reuse the same subjects and question-generation engine behind a goal-dashboard (Middle) or XP/level "premium tween" (Senior) presentation — see §26 for the full tier breakdown.
...
- **Clock robustness [Shipped]:** streaks and the daily hello bonus key off the local calendar date, and date anomalies always resolve in the child's favor — a clock moved backward never revokes an earned streak or bonus, at most one hello bonus is granted per calendar date, and DST or timezone travel never breaks a streak. (`todayStr`/`dayDiff` compare local `YYYY-MM-DD` date strings, immune to DST; §17.1 test 13 covers this — stale duplicate tag corrected here.)
...
| `index.html` | All 27 design mocks + 3 art variations on one canvas — **stale** as of 2026-07-21: not regenerated to include the §26 age-tier screens (no build step produces it from `index.src.html`); `index.src.html` is current |
...
**Shipped in `app.html`.** O8 (Appendix B) was ratified: Bloom Academy now serves ages 5–12 via three tiers, auto-selected from `profile.age` (§26.1), reusing the existing 4 subjects and question-generation engine rather than the design mock's illustrative placeholder subjects. Source: an updated design-canvas export (2026-07-21) — `screens-h.jsx`, `screens-i.jsx`, a `.senior` scope added to `styles.css` — reconciled into the live build the following day.
...
|---|---|---|---|
| **Junior** | 5–7 | Existing Bloom Academy system (§6): cream/coral/leaf palette, Fredoka, sticker cards, Pip as a full-size companion, map/node progression. | **Shipped**, unchanged — every §26 addition is additive and the full Junior acceptance suite (62 tests) passes unmodified. |
...
- **Typed-answer word problems** (`WordProblemView`, content.js `wordProblems` bank, 8 items across difficulty 1–3): free-text entry via an on-screen keypad, checked against `acceptEquivalents` (e.g. "2.25", "2 1/4", "9/4" all accepted for the same value) rather than exact-string match.
- **Reading comprehension** (`ComprehensionView`, content.js `comprehensionPassages` bank, 3 passages × 3 questions): a full passage with paired multiple-choice questions, graded as one activity-loop "question" (whole-passage pass/fail feeds `TIER_ANSWER`), matching the design mock's per-passage Q-of-3 progress track.
...
### 26.6 Known gaps / follow-ups

- **No achievements/badge screen.** The design mock's `SeniorAchievementsScreen` (level bar + 8 badges) was not built — Senior currently has home, activity loop, and results, but no dedicated badges gallery. Level and XP are visible on the home dashboard; discrete unlockable badges are not tracked anywhere in state.
- **No Middle-tier activity-loop art distinction from Senior's mechanics.** Middle reuses Senior's exact stage lineup shape and the same new mechanics (word problems, comprehension) at an easier difficulty band (`minTier`/`maxTier` in content.js) — it does not have its own distinct "goal dashboard → simpler quiz" visual identity beyond the home screen; the activity loop itself looks like a lighter-themed version of Senior's.
- **`index.html`** (the compiled single-file design gallery) remains stale relative to `index.src.html`/`screens-h/i.jsx`, as noted when §26 was first drafted — unaffected by this implementation pass since it's a design reference, not the playable build.
- **Subject-set naming**: Middle/Senior home screens show "Numbers/Words/Science/Music," not the design mock's more grown-up subject names (Mathematics/Language/Science/Geography/Logic/History) — a deliberate simplification (§26.1) to avoid inventing content for subjects with no banks, flagged here in case a future content pass wants Senior-appropriate subject *naming* without new subject *content*.
...
5. ~~Content authoring cost for comprehension/word-problems?~~ **8 word problems, 3 comprehension passages shipped** as a starting bank (§26.4) — production-volume growth is a content-authoring follow-up, same pattern as §15.1's Junior bank-growth work, not further tracked as an open product decision.
