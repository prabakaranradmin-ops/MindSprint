...
**Doc version:** 4.11 · **Status date:** 2026-07-22 (see Changelog, §25)
...
- **Not built**: an achievements/badge system (the design mock's `SeniorAchievementsScreen`, screen 31) — tracked as an open follow-up, §26.6.
...
- **Middle/Senior can never earn stars**, only XP (`TIER_NEXT_Q` never touches `profile.stars`; only Junior's `NEXT_Q` does) — since `MiddleHomeScreen`/`SeniorHomeScreen` still filter Music out via the same `subjLocked` star-gate Junior uses (`MUSIC_UNLOCK_STARS`, 30 stars), **a child who starts directly in Middle or Senior tier can never unlock Music at all**. This is a real §14 "no broken promises" risk if Middle/Senior are meant to ever surface Music — flagged for a product decision (does Middle/Senior need their own XP-based Music unlock threshold, or is Music intentionally Junior-only?), not fixed here since it's a design call, not a clear bug.
- The Middle-tier stage-picker screen (`TierSubjectStagesScreen`) is functionally correct but visually thin next to Junior's map or Senior's card grid — large empty white cards with a title and one instruction line, no icon, no per-stage progress/status indicator.
- Reused Junior mechanics (addition, subtraction, compare, pattern) never actually get harder content at Middle/Senior tiers — `genAdditionQs`'s difficulty ceiling is identical at `adapt.tier>=2` whether tier is 2 (Middle) or 3 (Senior), so e.g. "Warm-Up Math" addition problems stay within roughly sums-to-9 for a 12-year-old exactly as they would for a 7-year-old. Only the two genuinely new mechanics (word problems, comprehension) scale by tier via `minTier`/`maxTier`. Same root limitation §3.2 already documented for World 2's reused-generator content.
- The word-problem "Coach tip" hint is always fully worked (e.g. states the exact operation and numbers to multiply), not a partial nudge — arguably undercuts the stage's own "read carefully" framing since a child could solve from the hint without reading the problem. Worth a design look if hint strength becomes a concern; not changed here since §14 already mandates hints never penalize and this doesn't violate that, it's a strength calibration question.
...
| O10 | Product owner | **Middle/Senior Music unlock is currently unreachable** (found in §26.9 playtesting): Middle/Senior tiers never award `profile.stars` (only XP), but `MiddleHomeScreen`/`SeniorHomeScreen` still gate Music behind the same 30-star threshold Junior uses. A child who starts directly in Middle or Senior can never unlock Music. Needs a decision: give Middle/Senior their own XP-based Music unlock, treat Music as intentionally Junior-only and hide/relabel it for older tiers, or something else — not fixed in code since it's a design call, not an unambiguous bug. | Not gating; should resolve before Middle/Senior is considered feature-complete |
